      ...
      if (!kvs.isEmpty()) {
        for (Cell c : kvs) {
          ...
          // disk.
        	...
        	System.out.println("************ come in method performFlush in StoreFlusher class "+c.getTypeByte()+","+Bytes.toString(CellUtil.cloneRow(c))+","+Bytes.toString(CellUtil.cloneFamily(c))+","+Bytes.toString(CellUtil.cloneQualifier(c))+","
        			+Bytes.toString(CellUtil.cloneValue(c))+","+c.getTimestamp());
        	
          sink.append(c);
        }
        kvs.clear();
      }
      System.out.println("************ come in method performFlush in StoreFlusher class, the cell is empty ");
    } while (hasMore);
  }
}
